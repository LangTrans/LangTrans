            ...
            if w in value:
                ...
                if "replace" in value[w]:
                    replas.update(
                        {
                            w: [
                                (comp(q[0]), q[1]) if 1 < len(q) else (comp(q[0]), "")
                                for q in value[w]["replace"]
                            ]
                        }
                    )
...
def matching(patterns, cats, content):
    ...
    for (name, pattern), cat in zip(patterns.items(), cats):
        ...
        if len(cat) == 1:
            ...
            continue
        matches[name] = [
            {c: i for i, c in zip(mat, cat)} for mat in pattern.findall(content)
        ]
...
def main(content, exlocation, plocation):
    ...
    fdict, pt, cats = extract(spattern)
    del spattern
    count = 0
    while 1:
